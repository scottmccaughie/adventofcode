...
(defn valid-1?
  [passphrase]
  (let [constituents  (string/split passphrase #" ")
        unique-constituents (into #{} constituents)]
    (= (count constituents) (count unique-constituents))))

(defn valid-2?
  [passphrase]
  (let [constituents  (string/split passphrase #" ")
        unique-constituents (into #{} (map sort constituents))]
    (= (count constituents) (count unique-constituents))))

(defn part-1
  ""
  []
  (count (filter valid-1? passphrases))
...
(defn part-2
  ""
  []
  (count (filter valid-2? passphrases)))